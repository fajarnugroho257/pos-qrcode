...
<div>
    <main class="max-w-7xl mx-auto py-8 px-4">
        <div class="bg-white p-8 md:p-10 rounded-2xl shadow-sm border border-gray-200 min-h-100">
            ...
            <form action="{{ route('dataUserAddProcess') }}" method="POST" >
                @method('POST')
                @csrf
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <input type="text" name="name" value="{{ old('name') }}" placeholder="Nama Lengkap" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div class="relative">
                                        <select id="role" name="role_id"
                                            ...
                                            @foreach ($rs_role as $roles)
                                                <option @selected(old('role_id') == $roles->role_id) value="{{ $roles->role_id }}">{{ $roles->role_name }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="space-y-2 group">
                                    ...
                                    <div class="relative">
                                        <select name="user_st" name="user_st"
                                            ...
                                            <option @selected(old('user_st') == 'yes') value="yes">Aktif</option>
                                            <option @selected(old('user_st') == 'no') value="no">Non Aktif</option>
                                        </select>
                                        <div class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none text-gray-400 group-focus-within:text-indigo-500 transition-colors">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path d="M19 9l-7 7-7-7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" name="username" value="{{ old('username') }}" placeholder="Username" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                            </div>
                            <div class="space-y-2">
