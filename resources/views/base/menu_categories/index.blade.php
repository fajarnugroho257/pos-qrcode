        ...
        <div class="bg-white p-8 md:p-10 rounded-2xl shadow-sm border border-gray-200 min-h-100">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                ...
                <div class="flex items-center gap-3 justify-between">
                    {{-- Search --}}
                    <form action="{{ route('menuCategories') }}" method="GET">
                        <div class="relative">
                            <input type="text"
                                   name="search"
                    ...
                    {{-- Add --}}
                    <a href="{{ route('menuCategoriesCreate') }}"
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50/50 text-gray-600 text-xs uppercase tracking-wider text-center">
                            ...
                            <th class="px-6 py-4 font-semibold border-b border-gray-100">Nama Kategori</th>
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm md:text-base">
                        @forelse ($categories as $key => $category)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a href="{{ route('menuCategoriesEdit', $category->id) }}"
                                       class="text-gray-400 hover:text-indigo-600 p-1">
                                        <i class="fas fa-pen-to-square"></i>
                                    </a>
                                    <form action="{{ route('menuCategoriesDelete', $category->id) }}"
                                        method="POST"
