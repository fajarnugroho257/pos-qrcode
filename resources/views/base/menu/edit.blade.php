...
<div>
    <main class="max-w-7xl mx-auto py-8 px-4">
        <div wire:key="menu-form-" class="bg-white p-8 md:p-10 rounded-2xl shadow-sm border border-gray-200 min-h-100">
            ...
            <form action="{{ route('menuAppEditProcess', ['menu_id' => $detail->menu_id]) }}" method="POST">
                @method('POST')
                @csrf
                <div class="space-y-8 mb-4">
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <input type="text" name="menu_name" value="{{ old('menu_name', $detail->menu_name) }}" placeholder="Contoh: Setting Menu" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                            </div>
                            <div class="space-y-2">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" name="menu_icon" value="{{ old('menu_icon', $detail->menu_icon) }}" placeholder="fas fa-cogs" class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                                </div>
                            </div>
                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-700">URL / Route</label>
                                <input type="text" name="menu_url" value="{{ old('menu_url', $detail->menu_url) }}" placeholder="dashboard/setting" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                            </div>
                                ...
                                <div class="space-y-2 group">
                                    ...
                                    <div class="relative">
                                        <select name="menu_st" id="menu_st" class="w-full appearance-none px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 outline-none ring-offset-1 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all cursor-pointer">
                                            <option value="">-</option>
                                            <option @selected(old('menu_st', $detail->menu_st) == 'yes') value="yes">Aktif</option>
                                            <option @selected(old('menu_st', $detail->menu_st) == 'no') value="no">Non Aktif</option>
                                        </select>
                                    ...
                                    <div class="relative">
                                        <select name="menu_parent" id="menu_parent" class="w-full appearance-none px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 outline-none ring-offset-1 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all cursor-pointer">
                                            @foreach ($rs_option as $option)
                                                <option @selected(old('menu_parent', $detail->menu_parent) == $option['id']) value="{{ $option['id'] }}">{{ $option['name'] }}</option>
                                            @endforeach
                                        </select>
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-6 text-indigo-600">
                            ...
                            <h3 class="font-bold uppercase tracking-wider text-xs">Struktur & Level</h3>
                        </div>
                ...
                <div class="bg-gray-50 px-8 py-6 flex items-center justify-end gap-3 border-t border-gray-100">
                    ...
                    <button type="submit" class="px-8 py-2.5 bg-indigo-600 text-white cursor-pointer text-sm font-bold rounded-md md:rounded-lg shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all active:scale-95">
                        Simpan
                    </button>
                </div>
            </form>
