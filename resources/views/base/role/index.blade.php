        ...
        <div class="bg-white p-8 md:p-10 rounded-2xl shadow-sm border border-gray-200 min-h-100">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                ...
                <div class="flex items-center gap-3 justify-between">
                    <form action="{{ route('rolePengguna') }}" method="get">
                        <div class="relative">
                    ...
                    <a href="{{ route('rolePenggunaAdd') }}" class="cursor-pointer bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-md md:rounded-lg text-xs md:text-sm font-medium transition-colors flex items-center gap-2">
                        ...
                        @foreach ($rs_role as $key => $role)
                            <tr class="hover:bg-gray-50/65 transition-colors">
                                ...
                                <td class="px-6 py-4 flex justify-center">
                                    <a href="{{ route('rolePenggunaEdit', ['role_id' => $role->role_id]) }}" class="text-gray-400 hover:text-indigo-600 p-1"><i class="fas fa-pen-to-square"></i></a>
                                    <a href="{{ route('rolePenggunaDeleteProcess', ['role_id' => $role->role_id]) }}" onclick="return confirm('Apakah anda yakin akan menghapus data ini ?')" class="text-gray-400 hover:text-red-600 p-1"><i class="fas fa-trash-can"></i></a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
