...
<div>
    <main class="max-w-7xl mx-auto py-8 px-4">
        <div wire:key="menu-form-" class="bg-white p-8 md:p-10 rounded-2xl shadow-sm border border-gray-200 min-h-100">
            ...
            <form action="{{ route('rolePenggunaAddProcess') }}" method="POST">
                @method('POST')
                @csrf
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <input type="text" name="role_name" value="{{ old('role_name') }}" placeholder="Nama Role" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                            </div>
                        </div>
                    </div>
                </div>
