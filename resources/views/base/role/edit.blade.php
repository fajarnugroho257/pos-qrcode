        ...
        <div wire:key="menu-form-" class="bg-white p-8 md:p-10 rounded-2xl shadow-sm border border-gray-200 min-h-100">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                ...
                <div class="flex justify-end gap-3">
                    <a href="{{ route('rolePengguna') }}" class="cursor-pointer bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-md md:rounded-lg text-xs md:text-sm font-medium transition-colors flex items-center gap-2">
            ...
            <form action="{{ route('rolePenggunaEditProcess', $detail->role_id) }}" method="POST">
                @method('POST')
                @csrf
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <input type="text" name="role_name" value="{{ old('role_name', $detail->role_name) }}" placeholder="Nama Role" class="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                            </div>
                        </div>
                    </div>
                </div>
